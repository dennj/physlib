...
theorem posPart_eq_zero_iff : A⁺ = 0 ↔ A ≤ 0 := by
  ...
  · intro hA
    ...
    have hinner : ⟪-A, A⁺⟫ = 0 := by
      simpa [hnegPart] using (inner_negPart_zero_iff (A := -A)).2 hA'
    have hdecomp : -A = A⁻ - A⁺ := by
      calc
        -A = -(A⁺ - A⁻) := by rw [posPart_add_negPart A]
        _ = A⁻ - A⁺ := by simp [sub_eq_add_neg, add_comm]
    have hself : ⟪A⁺, A⁺⟫ = 0 := by
      rw [hdecomp, HermitianMat.inner_sub_right, HermitianMat.inner_comm,
        posPart_inner_negPart_zero, zero_sub] at hinner
...
theorem posPart_le_zero_iff : A⁺ ≤ 0 ↔ A ≤ 0 := by
  constructor
  · intro h
    exact (posPart_eq_zero_iff (A := A)).mp <| le_antisymm h (posPart_nonneg A)
  · intro hA
    simp [(posPart_eq_zero_iff (A := A)).2 hA]
