...
theorem piProd {Λi : ∀ i, MatrixMap (dI i) (dO i) R} (h₁ : ∀ i, (Λi i).IsTracePreserving) :
  ...
  · subst hfg
    have hprod :
        (∑ x : ∀ i, dO i, ∏ i, (Λi i).choi_matrix (x i, f i) (x i, f i)) =
          ∏ i, ∑ x, (Λi i).choi_matrix (x, f i) (x, f i) := by
      simpa using
        (Fintype.prod_sum (f := fun i x => (Λi i).choi_matrix (x, f i) (x, f i))).symm
    rw [hprod]
    have hdiag : ∀ i, ∑ x, (Λi i).choi_matrix (x, f i) (x, f i) = 1 := by
      intro i
      simpa [Matrix.traceLeft] using congrFun₂ (htrace i) (f i) (f i)
    simp [hdiag]
  · have hi : ∃ i, f i ≠ g i := by
      simpa [funext_iff] using hfg
    rcases hi with ⟨i, hi⟩
    have hfactor : ∑ x, (Λi i).choi_matrix (x, f i) (x, g i) = 0 := by
      have := congrFun₂ (htrace i) (f i) (g i)
      simpa [Matrix.traceLeft, Matrix.one_apply, hi] using this
    have hprod :
        (∑ x : ∀ i, dO i, ∏ i, (Λi i).choi_matrix (x i, f i) (x i, g i)) =
          ∏ i, ∑ x, (Λi i).choi_matrix (x, f i) (x, g i) := by
      simpa using
        (Fintype.prod_sum (f := fun j x => (Λi j).choi_matrix (x, f j) (x, g j))).symm
